...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(
        ...
        GRVTILD, KC_1,    TWOAT,   THREEHSH,KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC,   KC_INS,  KC_HOME, KC_PGUP, \
        KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC, KC_NUBS,   KC_DEL,  KC_END,  KC_PGDN, \
        KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, QUODQUO, KC_ENT, \
...
uint16_t reg_unshifted_or_shifted(uint16_t kc, uint16_t unshifted_keycode, uint16_t shifted_keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        if (get_mods() & MOD_MASK_SHIFT) {
            kc = shifted_keycode;
        } else {
            kc = unshifted_keycode;
        }
        register_code(kc);
        } else {
        unregister_code(kc);
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        {
            static uint16_t kc;
            kc = reg_unshifted_or_shifted(kc, KC_GRV, UK_TILD, record);
            return false;
        }
        case TWOAT:
        {
            static uint16_t kc;
            kc = reg_unshifted_or_shifted(kc, KC_2, UK_AT, record);
            return false;
        }
        case THREEHSH:
        {
        ...
        {
            static uint16_t kc;
            kc = reg_unshifted_or_shifted(kc, UK_QUOT, UK_DQUO, record);
            return false;
        }
        case L_BRI:
            if (record->event.pressed) {
